...
final case class AnimatedEntity(currentStep: Int, vertexArrayObjectIds: List[Int], textureId: Int, size: List[Int]) {
...
object AnimatedEntity {
  ...
  def createFrom(filenames: List[String], textureId: Int): AnimatedEntity = {
    val tuple = filenames
    ...
    AnimatedEntity(0, vertexArrayObjectIds, textureId, sizes)
  }
}
